...
import os
import math
from collections import Counter #集合模块,Counter计数器，是一个字典，比如用来统计某个字符在字符串出现的次数
...
def PivotList(x):
    ...
    #不是list的元素也转换成list，最后得到一个List，里面的元素都是list
    if type(x[0]) == type([1]):   # type([1])返回的就是list
        newList = list()
        dim1 = len(x[0])
...
def ReadInCellLineData(filename, opFilename):
    ...
    with open(filename, 'rb') as csvfile:
        ...
        rowCtr = 0
        for row in reader: #row是list
            if rowCtr == 0: #处理第一行数据（表头）
                ...
                for jj in range(len(row)):
                    if jj == 0:
                        gene.append(row[0]) #基因名称
                    else:
                        curRowVals.append(int(row[jj])) #获取当前基因对应的各个值，添加到list    
    ...
    pickle.dump(PivotList(mutVal), f0)
    pickle.dump(cellLine, f0)
    pickle.dump(gene, f0)
    f0.close()
...
'''
params: numDrugs #每个对比组所包含的药物种数
opFilename: 输出文件名称
注意每次传入的是不同的feature文件 AUC.csv OR IC50.csv
'''
def ReadPharmacologyData_AZD9291(filename, opFilename, numDrugs):
    ...
    #     drugSens.append([]) # 如两种药物进行对比 drugSens = [[], []] 二维数组
    # drug2Sens = list()
    # drug3Sens = list()
    sensData = list()
        ...
        for row in reader:
            if rowCtr == 0:
                ...
                for jj in range(len(drugName)):
                    drugNameColInd.append(header.index(drugName[jj])) #获取当前drug在表头中的列的index
                ...
                sensData.append(curRow) #当前cell line对所有药物的反应数据 sensData二维数组,第一维是cell line, 第二维是对药物的反应值(features)

                # Capture cell line in its own list
                # 5-5-16:  Do conversion of cell lines names to
                # remove hyphens
                cellLine.append(row[0].replace('-','').upper()) #空格替换下划线
    ...
    for ii in range(numDrugs):
        ...
        pickle.dump(header[drugNameColInd[ii]],f0) #药物名称
        pickle.dump(cellLine, f0) # Cell Line
        # cPickle.dump(tissue, f0)
        pickle.dump(drugSens[ii], f0) #二维数组 drugSens[0]是所有cell line对Drug1的IC50(或AUC)反应值，[1]是对Drug2的反应值
        f0.close()

    # return header, cellLine, tissue, drugSens, drugSensFileNames
    '''
    header: 表头
    cellLine: 所有cell line的名称
    drugSens： 对当前drugName中的各种药物的反应数据
    drugSensFileNames: 保存文件名称
    '''
    ...
    with open(filename, 'r') as csvfile:
        ...
        for row in reader:
            if rowCtr == 0:
                ...
                if rowCtr<3:
                    print('cellLine: ', curRow[1], 'cosmicID: ', curRow[2], 'studyAbb: ', curRow[3], 'cancerType: ', curRow[4])
                rowCtr += 1
...
def ReadTargetsFile():
    ...
    """
    compList: 二维列表 [['AZ628', 'RDEA119-2'], ['AZ628', 'CI-1040'], ['AZ628', 'PD-0325901'],
    ['PLX4720-2', 'RDEA119-2'], ['PLX4720-2', 'CI-1040'], ['PLX4720-2', 'PD-0325901'], ['SB590885', 'RDEA119-2'],
    ['SB590885', 'CI-1040'], ['SB590885', 'PD-0325901']]
    """
    ...
    targList = list()
    compList = list()
    with open(wrapperFile[0], 'r') as csvfile:
        reader = csv.reader(csvfile, delimiter=',')
        for row in reader:
...
def IterateOverTargets(targList, compList, Ntargets):
    ...
    for ii in range(len(targList)):
        ...
        drugName = list() #注意： 每一组药物对比完后初始化drugName
        ...
        for jj in range(numDrugs):
            drugName.append(compList[ii][jj]) # 当前target的各种药物名称(第i组对比target的第j个药物名称)
            for kk in range(numGeneAlterationTypes): #依次取出该feature的名称，目前只使用了AUC和IC50
                # drugSensitivityType = ['AUC', 'IC50']
                # drugSensitivityType[0] = 'AUC'
                # drugSensitivityType[1] = 'AUC'
...
def MakeMultivariatePatientNetworkGraph(valuesList, patientIdList):
    ...
    for ii in range(len(allVarsInclType)):
        ...
        # list的长度相同, 符合条件的ele(xx)的index(jj)直接append到数组
    ...
    if sum(varsUsedList) > 0 and len(patientIdList) > 2:
        # At least 1 variables had diversity(多样性)
...
def PrepareCellLineNetworkData():
    ...
    # Save each drug response to a separate file with the same metadata
    drugSensFile = list()
    for ii in range(numDrugSensitivityTypes): # numDrugSensitivityTypes, 特征数量，目前只有IC50和AUC
        for jj in range(numDrugs):
...
#     print(drugSensFile)
    ...
    cellLineOrig = list()
    cellLinePh = list()
    drugSensPh = list() #前面的特征向量组合形式[AUC_DRUG_1, AUC_DRUG_2, IC50_DRUG_1, IC50_DRUG2]
    numFiles = len(drugSensFile) #numFiles = 4
#     print(drugSensFile)
    for ii in range(numFiles):
        if ii == 0:
            f0 = open(drugSensFile[0],'rb') #读取在ReadPharmacologyData_AZD9291存贮的data文件
#             f1 = open(drugSensFile[0],'rb')
#             print(base64.b64encode(f1.read()))

            curDrugNamePh = pickle.load(f0) #读取drug name
#             print('\n curDrugNamePh: ', curDrugNamePh)
            cellLineOrig.append(pickle.load(f0))
#             print(cellLineOrig[0])
#             print('\n cellLineOrig: ', cellLineOrig)
            cellLinePh = cellLineOrig[0] #读取cell line
#             print('重复统计: ', Counter(cellLinePh))
#             print('\n cellLinePh: ', cellLinePh)
            # curTissuePh = cPickle.load(f0)

            drugSensPh.append(pickle.load(f0)) #读取feature值
#             print('\n drugSensPh: ', np.array(drugSensPh).shape)

            # print(str(ii) + ' :  number of cell lines is ' + str(len(cellLinePh)))
            f0.close()
        else:
            f0 = open(drugSensFile[ii],'rb')
            curDrugNamePh = pickle.load(f0)
            cellLineOrig.append(pickle.load(f0))
            # curTissuePh = cPickle.load(f0)
            drugSensPh.append(pickle.load(f0))
            f0.close()
#             print(np.array(cellLineOrig).shape) 最后共4行1074列
#             print(np.array(drugSensPh).shape) 最后共4行1074列

#             print('cellLinePh shape-> ', np.array(cellLinePh).shape, ' Orig Shape-> ', np.array(cellLineOrig[ii]).shape)
#             print('set = ', len(set(cellLinePh)), '---',len(set(cellLineOrig[ii])))
            cellLinePh = sorted(list(set(cellLinePh).intersection(set(cellLineOrig[ii]))))
#             print('cellLinePh set sorted shape', np.array(cellLinePh).shape)
            # print(str(ii) + ' :  number of cell lines is ' + str(len(cellLinePh)))

#     print(np.array(cellLinePh).shape) # 1065,
#     print(np.array(cellLineOrig).shape) # 4,1074
#     print(np.array(drugSensPh).shape) # 4, 1074
#     print(drugSensPh[0])
    ...
    for ii in range(len(cellLinePh)):
        ...
        # (cellLinePh).shape   (1065,) 
        # (cellLineOrig).shape  (4, 1074) 第一维代表Drug N, 第二维代表 Cell Line
        # (drugSensPh).shape   (4, 1074)  第一维代表Drug N, 第二维代表 Feature值
        for kk in range(numDrugs):
            # Find corresponding entry in cellLineOrig[kk]
            curInd = [ mm for mm,xx in enumerate(cellLineOrig[kk]) if xx == cellLinePh[ii]][0] #相匹配的cell line
            if drugSensPh[kk][curInd] == 'NA' or drugSensPh[kk+numDrugs][curInd] == 'NA': # IC50或AUC的值为NA，只要任意一个feature值为NA就舍弃
                noNas = False
    ...
    print('-'*10, 'Extract drug sens and cell line info from AUC.csv and IC50.csv', '-'*10)
    print('Valid drugSens: ', np.array(drugSensVals).shape) #shape 862
    print('Valid Cell Lines: ', np.array(cellLinePh).shape) #shape (2, 862, 2) 第一维是第n个drug， 第二维是cell line,第三维是feature
    print('-'*40)
    #     print(drugSensVals[0])
    # drugSensVals: [[0.98015968, 2.023364598], [0.9595672, 1.58908172], ....]


    # 只保留有明确tissueType的cell line
...
#     print(studyAbb)
    ...
    for ii in range(len(cellLinePh)): #cellLinePh 通过AUC和IC50.csv取出的cell line list
        ...
        if cellLineInfo.count(cellLinePh[ii]) > 0: #如果cellLineInfo中有cellLinePh中出现的cell line
            matchInd = cellLineInfo.index(cellLinePh[ii]) #取出该cell line在cellLineInfo中的index
            if studyAbb[matchInd] in tissType or tissType[0] == 'ALL': # tissType定义在Infofile.py中
    ...
    print('-'*10, 'Mathing Cell line info.csv', '-'*10)
    print('cellLineTissPh Shape: ', np.array(cellLineTissPh).shape)
    print('studyLineAbbPh Shape: ', np.array(studyLineAbbPh).shape)
    print('drugSensValsTiss Shape: ', np.array(drugSensValsTiss).shape)

#     print(Counter(studyLineAbbPh))
    ...
    print('-'*10, 'Replace - in cell line name with space')
    print('cellLineTissPhFix reshape: ', np.array(cellLineTissPhFix).shape, '\n')
#     print(cellLineTissPhFix)
    ...
    print('Shape: \n')
    print('cellLinesX -> ', np.array(cellLinesX).shape, '\n')
    print('drugsSensValsX -> ', np.array(drugSensValsX).shape, '\n')
    print('studyLineAbbX -> ', np.array(studyLineAbbX).shape, '\n')

    print(Counter(studyLineAbbX))
...
def RebuildGList():
    ...
    iterBinPartList = pickle.load(f0)
    gListGraphNameList = pickle.load(f0)
    gListNumVertsList = pickle.load(f0)
    gListVarsUsedList = pickle.load(f0)
    gListClosure = pickle.load(f0)
...
def PartitionGraphAndSegment(g, parentLabel):
    ...
    #  只有方阵才有特征向量，因此对于非方阵，只考虑SVD分解
    #  这里只考虑符号的问题
    # https://zh.wikipedia.org/wiki/%E4%BB%A3%E6%95%B0%E8%BF%9E%E9%80%9A%E5%BA%A6#Fiedler%E5%90%91%E9%87%8F
    fVec = u[:,len(s)-2]
    fVal = s[len(s)-2]
    ...
    for jj in range(len(fVec)):
        if fVec[jj] > 0.0:
            ...
            binFVec.append(0)
    if sum(binFVec) > math.floor(len(binFVec)/2.0): # why?
...
def ExtractSubpopulations(gList, gListClosure):
    ...
    for ii in range(len(gListClosure)):
        ...
        if gListClosure[ii] == 0:
            # Current graph is a constituent graph; so keep track of it and its parameters #叶节点图
            gListInd.append(ii)
...
def EvaluateSubpopDist():
    ...
    :return:
    """
    # 用log(IC50)/AUC 表示一个新的符合变量

    # Load subpop results
    f = open(localResultsFolder + expTag + curTarg + '_'  + 'SubpopResults', 'r')
    numVerts = pickle.load(f) #1D List, 存每个graph的顶点数
    numVarsUsed = pickle.load(f) # 1D List, 存得到当前grapgh的数据的变量数
    gName = pickle.load(f) #
    varsUsed = pickle.load(f)  # 2D list, 使用的变量名?
    clinVals = pickle.load(f) # 2D list， 每个graph的drug sensitivity
    cellLines = pickle.load(f) # 2D list, 每个graph的Cell lines
    gListClosure = pickle.load(f) # 1D list， 每个graph的closure状态
    f.close()
...
def CellLine_Master_Method():
    ...
    '''
    二维数组
    [i][0]是路径，[i][1]是feature名称
    drugSensitivityFile[0] = [dataRoot + 'public_pharmacology_AUC_alpha.csv', 'pharmDataAUC']
    drugSensitivityFile[1] = [dataRoot + 'public_pharmacology_IC50_alpha.csv', 'pharmDataIC50']
    '''
