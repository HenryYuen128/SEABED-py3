...
class Node:
    ...
    def set_visual_attributes(self, colors, xlab):
        ...
        if self.parent == None:
            self.color = 'cyan' if self.outcome == None else colors[self.outcome]
        if self.is_leaf():
            perc = '.3' if len(xlab) == 1 else '.35'
            self.color = 'cyan' if self.outcome == None else \
                "black;{}:{}".format(perc, colors[self.outcome])
        # outline color
        self.outline = 'black'
        # shape
        self.shape = 'oval' if self.is_leaf() else 'box'
        # label
